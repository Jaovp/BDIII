...
import com.example.meusgastos.domain.repository.TtituloRepository;

@Service
public class TituloService implements ICRUDService<TituloRequestDTO, TituloResponseDTO> {

    @Autowired
    private TtituloRepository tituloRepository;
    @Autowired
    ...
    private void validarTitulo(TituloRequestDTO dto) {
        if(dto.getTipo() == null || dto.getDataVencimento() == null || dto.getValor() == null || dto.getDescricao() == null) {
            throw new BadRequestException("Título Inválido - Os campos tipo, dataVencimento, valor e descrição são obrigatórios");
        } 
    }
